    ...
    if(mode == 1){  // 普通模式
        ...
        for(int i = 0; i < 100; i ++){
            threads.emplace_back([](){
                ...
                testsession(socket, 1000);  // 每个线程发送1000条消息
            });
            std::this_thread::sleep_for(std::chrono::milliseconds(10)); // 为了模拟高并发，线程之间有10毫秒的间隔
        }
...
void testsession(boost::asio::ip::tcp::socket &socket, int times){
    try{
        ...
        for(int i = 0; i < times; i++){
            ...
            // 将发送数据封装成 CSendMsgNode
            CSendMsgNode send_msg_node(1001, msg_data);
